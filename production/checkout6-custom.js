function paymentUpdateCode(orderForm) {
	...
	try {
		fetch(`/api/checkout/pub/orderForm/${orderFormId}/customData/custom-payment/customPaymentId`, {
			...
			body: JSON.stringify({
				"expectedOrderFormSections": ["items", "gifts", "totalizers", "clientProfileData", "shippingData", "paymentData", "sellers", "messages", "marketingData", "clientPreferencesData", "storePreferencesData", "customData"],
				"value": paymentCode
			})
		}).then((response) => {
...
function clearCartUser() {
	if (!$('.clear-cart').length) { //$('#cart-title').append('<button class="clear-cart"> Limpar carrinho </button>');
	}
	...
	80: function (t, e) {
		...
		var a = {
				...
				formatMoney: function (t, e, r) {
					...
					}(n, o) : "".concat(Math.round(n))).split(".");
					return c[0].length > 3 && (c[0] = c[0].replace(/\B(?=(?:\d{3})+(?!\d))/g, i)),
						("" [1] || "").length < o && (c[1] = c[1] || "",
			...
			c = {
				start: function (t) {
					this.addHtmlFlag(),
						this.fetchCredit(t),
						this.creditMessagePayment()
				},
				addHtmlFlag: function () {
					var t = '<div class="custumer-credit">\n            <p class="credit__text">\n                Limite de crédito:\n\n                <strong class="credit__text-price">\n                    R$ 0,00\n                </strong>\n            </p>\n        </div>';
					$("h1#orderform-title").append(t),
						$("body.body-cart #cart-title").prepend(t),
						$(".orderform-template .mini-cart .custumer-credit").length || $(".orderform-template .mini-cart").prepend(t)
				},
		...
		$(document).ready((function () {
			$(window).on("orderFormUpdated.vtex", (function (t, e) {
				...
				if (e.clientProfileData) {
					var o = function (t) {
							for (var e = "".concat(t, "="), r = decodeURIComponent(document.cookie).split(";"), n = 0; n < r.length; n++) {
								for (var o = r[n];
									" " === o.charAt(0);)
									o = o.substring(1);
								if (0 === o.indexOf(e))
